...
# Genres by country function (only for top 5 countries)
def genres_by_country(df):
    ...
    df_top5['genres'] = df_top5['listed_in'].str.split(',').apply(lambda x: [i.strip() for i in x])

    # Explode the genres column so that each genre has its own row
    df_expanded = df_top5.explode('genres')
    ...
    ax.set_title('Distribution of Genres by Country (Top 5 Countries)')
    st.pyplot(fig)
