...
def choropleth_map(df):
    ...
    # Clean the 'country' column to remove any leading/trailing spaces and handle missing data
...
if df_cleaned is None or df_cleaned.empty:
    ...
    col1, col2, col3 = st.columns([1, 3, 1])  # Adjust the column size as needed

    # In the first column, show the sidebar graph selection (already in sidebar)
    with col1:
        st.empty()  # First column empty (sidebar already takes care of this)

    # In the second column, show the selected graph (Choropleth map)
    with col2:
        if selected_graph == "Choropleth Map: Titles by Country":
            choropleth_map(df_cleaned)

    # In the third column, display the additional information (fixed table)
    with col3:
