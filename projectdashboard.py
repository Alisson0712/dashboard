...
with st.sidebar:
    ...
    graph_options = [
        ...
        "Distribution of Genres in the US"
    ]
...
# Distribution of genres by country (US only)
def genres_by_us(df):
    if df is None or df.empty:
        return
    ...
    df_us['genres'] = df_us['listed_in'].str.split(',').apply(lambda x: [i.strip() for i in x])  # Split genres

    # Explode the genres column so that each genre has its own row
    df_expanded = df_us.explode('genres')

    # Create a crosstab to count genres per country
    genre_us_count = pd.crosstab(df_expanded['genres'], df_expanded['country'])

    # Create the heatmap for US genres
    fig, ax = plt.subplots(figsize=(12, 8))
    sns.heatmap(genre_us_count, annot=True, cmap='Blues', ax=ax, cbar_kws={'label': 'Number of Titles'})
    ax.set_title('Distribution of Genres in the US')
    st.pyplot(fig)
...
if df_cleaned is not None:
    ...
    elif selected_graph == "Distribution of Genres in the US":
        genres_by_us(df_cleaned)