...
# Calculate top countries from df_reshaped
top_countries = df_reshaped['country'].value_counts().head(13)

# Create the bar chart using matplotlib
fig, ax = plt.subplots() # Create a figure and axes
top_countries.plot(kind='bar', ax=ax, title='Top 13 Content Producing Countries', color=colors)
ax.set_ylabel('Number of Titles') # Set y-axis label using the axes object
...
with col[0]:
    st.markdown('#### Movie vs TV Show Distribution by Country')  # Changed title

    # Calculate title counts for the top 13 countries
    top_13_countries = df_reshaped['country'].value_counts().head(13).index.tolist()
    ...
    # Display metrics for movie and TV show counts for each country
    for country in top_13_countries:
        ...
        st.write("---")  # Add a separator between countries

    # Display donut chart for all top 13 countries combined
...
# Coulmn 3
with col[2]:
    st.markdown('#### Top Genres (Top 13 Countries)')  # Changed title

    # Filter for top 13 countries
    top_13_countries = df_reshaped['country'].value_counts().head(13).index.tolist()
    filtered_df = df_reshaped[df_reshaped['country'].isin(top_13_countries)]

    # Group by genre and count titles for filtered data
