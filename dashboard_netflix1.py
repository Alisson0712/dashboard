...
# Usa las columnas correctamente
with col[0]:
...
# Ahora dentro de cada columna, debes usar el contenido correspondiente
with col[0]:
    st.markdown('#### Movie vs TV Show Distribution by Country')  # Título

    # Calcula los conteos de títulos para los 13 países principales
    top_13_countries = df_reshaped['country'].value_counts().head(13).index.tolist()
    ...
    # Muestra las métricas para películas y programas de TV por país
    ...
    # Muestra el gráfico de donut para los 13 países principales
    donut_chart = make_donut_chart(filtered_df, 'type')
    st.plotly_chart(donut_chart, use_container_width=True)

# Segunda columna
with col[1]:
    st.markdown('#### Netflix Titles Heatmap')  # Título
...
# Tercera columna
with col[2]:
    st.markdown('#### Top Genres (Top 13 Countries)')  # Título

    # Filtra para los 13 países principales
    top_13_countries = df_reshaped['country'].value_counts().head(13).index.tolist()
    filtered_df = df_reshaped[df_reshaped['country'].isin(top_13_countries)]

    # Agrupa por género y cuenta los títulos
    genre_counts = filtered_df['listed_in'].str.split(', ').explode().value_counts().reset_index()
    genre_counts.columns = ['Genre', 'Count']

    # Muestra los géneros más populares
    st.dataframe(genre_counts.head(10),  # Muestra los 10 géneros principales
